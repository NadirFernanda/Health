...
type Consultorio = {
  id: string; nome: string; email: string; morada: string; bairro: string;
  zonaLuanda: string; cidade: string; provincia: string; contacto: string;
  rating: number; totalAvaliacoes: number; totalSalas: number;
...
export default function AdminConsultorios() {
  ...
  useEffect(() => {
    fetch("/api/admin/consultorios", { credentials: "include" })
      .then(async (r) => {
        const body = await r.json().catch(() => ({}));
        if (!r.ok) throw new Error((body as { details?: string; error?: string }).details ?? `HTTP ${r.status}`);
        return body;
      })
