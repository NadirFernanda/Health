...
import { Check, X, Star, MessageCircle, MapPin, Ban, RotateCcw, UserCheck, Search } from "lucide-react";
...
export default function AdminClinicas() {
  ...
  const filtered = lista.filter((c) => {
    ...
    const matchSearch = !q || c.nome.toLowerCase().includes(q) || c.email.toLowerCase().includes(q) || (c.morada || c.zonaLuanda || "").toLowerCase().includes(q);
