...
import { plantoesMock, formatData } from "@/lib/mock-data";
import { TopBar } from "@/components/nav";
import { useRouter } from "next/navigation";
import { notFound } from "next/navigation";
import { Star, MapPin, Calendar, Banknote } from "lucide-react";

export default function AvaliarPlantaoMedico({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  // Só pode avaliar turnos concluídos (mock aceita todos para demo)
  // Em produção: if (plantao.estado !== "CONCLUIDO") return notFound();
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f7f8fa] px-6 text-center">
        ...
        <p className="text-gray-500 mt-2 text-sm leading-6">
          Obrigado pelo seu feedback. A sua avaliação ajuda a melhorar a qualidade das clínicas na plataforma.
        </p>
        ...
        <p className="text-gray-600 text-sm mt-3 italic">"{comentario || "Sem comentário"}"</p>
        <button
  ...
  return (
    <div>
      <TopBar titulo="Avaliar Turno" back={`/medico/plantoes/${id}`} />

      <div className="px-4 py-6 space-y-6">
        {/* Info do turno */}
        <div className="bg-white rounded-2xl border border-gray-100 p-4">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-2">Turno Realizado</p>
          <p className="font-semibold text-gray-900">{plantao.clinica.nome}</p>
          <p className="text-sm text-gray-500 mt-0.5 inline-flex items-center gap-1"><MapPin size={13} strokeWidth={1.75} /> {plantao.clinica.morada}, {plantao.clinica.cidade}</p>
          <p className="text-sm text-gray-500 mt-0.5 inline-flex items-center gap-1"><Calendar size={13} strokeWidth={1.75} /> {formatData(plantao.dataInicio)}</p>
          <p className="text-sm text-gray-500 mt-0.5 inline-flex items-center gap-1"><Banknote size={13} strokeWidth={1.75} /> {plantao.valorKwanzas.toLocaleString()} AOA</p>
        </div>

        {/* Avaliação — estrelas */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <p className="text-sm font-bold text-gray-700 text-center mb-1">Como foi a experiência na clínica?</p>
        ...
        {nota > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {["Pontualidade", "Equipamento", "Ambiente", "Organização", "Comunicação", "Remuneração"].map((tag) => {
        ...
        {nota > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            <textarea
              ...
              rows={4}
              placeholder="Partilhe a sua experiência neste turno..."
