...
import { validateFileMagicBytes, sanitizeFilename } from "@/lib/file-validation";
...
export async function POST(request: NextRequest) {
  ...
  const safeBasename = sanitizeFilename(file.name, ALLOWED_IMG_EXTS);
  if (!safeBasename) {
    return Response.json({ error: "Extensão de ficheiro não permitida." }, { status: 400 });
  }

  const filename = `foto-${Date.now()}-${safeBasename}`;
